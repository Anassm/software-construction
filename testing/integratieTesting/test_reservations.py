...
def test_update_reservation_success(auth_headers):
    ...
    res_id = res["id"]
    new_start = (datetime.utcnow() + timedelta(days=2)).isoformat()
    new_end = (datetime.utcnow() + timedelta(days=2, hours=4)).isoformat()
...
def test_update_reservation_not_found(other_headers, auth_headers):
    ...
    payload = {"startDate": (datetime.utcnow() + timedelta(days=5)).isoformat()}
...
def test_update_reservation_invalid_dates(auth_headers):
    ...
    res_id = res["id"]
    payload = {
        "startDate": (datetime.utcnow() + timedelta(hours=5)).isoformat(),
        "endDate": (datetime.utcnow() + timedelta(hours=4)).isoformat()
    }
