...
public class VehicleService : IVehicles
{
    ...
    public async Task<(int statusCode, object message)> DeleteVehicleAsync(string lid, string identityUserId)
    {
        try
        {
            ...
            var vehicle = await _dbContext.Vehicles.FirstOrDefaultAsync(v => v.ID.ToString().ToUpper() == lid.ToUpper() && v.UserID == user.ID || v.LicensePlate == lid.Replace("-", "") && v.UserID == user.ID || v.OldID == lid && v.UserID == user.ID);
            if (vehicle == null)
                return (404, new { error = "Vehicle not found!" });
